...
//             +----------------------------+
// C = channel
// Each node should have 3 interfaces pre-configured to 3 non-overlapping channels (wlan0:1, wlan1:4, wlan2:11)
//
...
// 1 - Set up the nodes:
//
// no action should be necessary if the node was setup using the testbed repo (https://github.com/regisin/testbed)
//
// 2 - Run the server side (replace wlan0 by one of the 3 installed):
//
// server host: $ ./waf --run="fd-emu-onoff --serverMode=1 --deviceName=wlan0"
//
// 3 - Run the client side (server is running, time it so both processes start approx. at the same time):
//       
// client host: $ ./waf --run="fd-emu-onoff --deviceName=wlan0"
//
